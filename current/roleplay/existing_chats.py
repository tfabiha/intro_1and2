...
def mainPage(email,magicNumber):
    ...
    #see example below:
    text = ''
    for files in os.listdir('../chats'):
         text += files.strip('.txt') + '<br>'
    
    return 'These are the chats that are available. Please type in the one that you want to enter.<br>' + text + '''

    <p>
		<form method="Get" action="roleplay_version1.py?''' + cgistuff + '''">
