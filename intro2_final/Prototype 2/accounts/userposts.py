...
def mainPage(email,magicNumber,user):
    ...
    posts = open('../posts/'+user+'.txt').read().split('<killeverything>')
    text = ""
    for i in posts:
        text = i + '<br>' + text
...
<p position="relative">
        ...
        <form method="Get" action="addtoposts_user.py" id="addpost">
                <input type="hidden" name="place" size="25" value="'''+user+'''">
		...
		<input type="submit" name="submited" value="post">
         ...
         <a href="world.py'''+cgistuff+userstuff+'''" class="ci1"><h5 id="ci">View Info</h5></a>
         <a href="posts.py'''+cgistuff+userstuff+'''" class="ba1"><h5 id="ba">Posts</h5></a>
...
def main():
    ...
    requiredFields = ['UserEmail','magicNumber','user']
    allHere = True
    ...
    print mainPage(email,magicNumber,user)            
main()
