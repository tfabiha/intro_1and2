...
def mainPage(email,magicNumber,comment,place):
    ...
    #print reader
    basic_html(email,magicNumber,place)
...
def basic_html(email,magicNumber,place):
    print '''<!DOCTYPE HTML>
    <html>
        <head>
            <script type="text/javascript">
                window.location.href = "userposts.py?UserEmail='''+email+'''&magicNumber='''+str(magicNumber)+'''&user='''+place+'''"
            </script>
            <title>Page Redirection</title>
        </head>
        <body>      
            If you are not redirected automatically, follow this <a href="userposts.py?UserEmail='''+email+'''&magicNumber='''+str(magicNumber)+'''&user='''+place+'''">Click here</a>.
        </body>
    </html>'''
